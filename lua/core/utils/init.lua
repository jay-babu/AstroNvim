    ...
    if run_me then
      ...
      vim.api.nvim_create_autocmd("User", {
        ...
        callback = function()
          ...
          vim.schedule(function() vim.notify "Mason is installing packages if configured, check status with :Mason" end)
        end,
      })
      vim.opt.cmdheight = 1
...
-- @param plugin the plugin string to call `requires` with
